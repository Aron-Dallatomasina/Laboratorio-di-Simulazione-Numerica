...
void Input(void) {
	...
	ig=5;						// Starting index for g(r)
	nbins=100;					// Number of bins for the histogram for g(r)
	bin_size=(0.5*box)/nbins;	// Length of each bin
	n_props=ig+nbins;			// Total number of observables
	// Resize arrays for g(r) histogram
	stima_g.resize(nbins);
	err_g.resize(nbins);
...
void Averages(int iblk) {
	...
	G.open("../Output/output_g.dat",ios::app);

	// Potential energy per particle
	//stima_pot=blk_av[iv]/blk_norm/(double)npart;															// Without tail correction (as in Es04_01/02)
	stima_pot=blk_av[iv]/blk_norm/(double)npart+(8.0*pi*rho/3.0)*(1.0/3.0/pow(rcut,9)-1.0/pow(rcut,3));		// With tail correction
	...
	//stima_etot=blk_av[ie]/blk_norm/(double)npart;															// Without tail correction (as in Es04_01/02)
	stima_etot=blk_av[ie]/blk_norm/(double)npart+(8.0*pi*rho/3.0)*(1.0/3.0/pow(rcut,9)-1.0/pow(rcut,3));	// With tail correction (same correction as the potential energy) 
	...
	// Pressure 
	//stima_pres=blk_av[ip]/blk_norm;																		// Without tail correction (as in Es04_01/02)
	stima_pres=blk_av[ip]/blk_norm+(32.0*pi*rho*rho)*(1.0/9.0/pow(rcut,9)-1.0/6.0/pow(rcut,3));				// With tail correction
	glob_av[ip]+=stima_pres;
	glob_av2[ip]+=stima_pres*stima_pres;
	err_press=Error(glob_av[ip],glob_av2[ip],iblk);
	// Radial distribution function
	for (int b=0; b<nbins; b++) {
		r_lower=b*bin_size;																					// Lower bound of the radial shell
		r_upper=r_lower+bin_size;																			// Upper bound of the radial shell
		deltaV=(4.0*pi/3.0)*(pow(r_upper,3)-pow(r_lower,3));												// Volume of the spherical shell
		stima_g[b]=(blk_av[ig+b]/blk_norm)/(rho*npart*deltaV);												// Block estimate of g(r) - normalized by ideal-gas reference density
		glob_av[ig+b]+=stima_g[b];
		glob_av2[ig+b]+=stima_g[b]*stima_g[b];
		err_g[b]=Error(glob_av[ig+b],glob_av2[ig+b],iblk);
	}
	...
	G<<setw(wd)<<iblk;
	for(int b=0; b<nbins; b++) G<<setw(wd)<<stima_g[b]<<setw(wd)<<glob_av[ig+b]/(double)iblk<<setw(wd)<<err_g[b];
	G<<endl;

	// Save final histogram of g(r)
	if(iblk==nblk) {
		ofstream Gfinal;
		Gfinal.open("../Output/output_gfinal.dat");
		for(int b=0; b<nbins; b++) {
			double r=(b+0.5)*bin_size;
			Gfinal<<setw(wd)<<r<<setw(wd)<<glob_av[ig+b]/(double)nblk<<setw(wd)<<err_g[b]<<endl;
		}
		Gfinal.close();
	}
...
void Measure() {
	...
	double vij;
	double dx,dy,dz,dr;
	int bin;										// Index of the histogram bin for g(r), corresponding to the distance dr
	for(int b=0; b<nbins; b++) walker[ig+b]=0.0;	// Reset accumulator for g(r) histogram
	...
	for (int i=0; i<npart-1; ++i) {
		...
		// Histogram for g(r)
			if(dr<0.5*box) {						// Consider distances up to box/2
				bin=static_cast<int>(dr/bin_size);	// Determine which radial bin the distance belongs to
				walker[ig+bin]+=2.0;				// Increment by 2 (each pair contributes to two particles)
			}
		}
	}
