...
// Generate a random number with linear distribution in [0,1) with angular coefficient m (-2<=m<=2)
double Random::LinearIn01(double m) {
	if (m>0) return (0.5-1.0/m)+sqrt(pow((0.5-1.0/m),2)+2.0*Rannyu()/m);
	else if (m<0) return (0.5-1.0/m)-sqrt(pow((0.5-1.0/m),2)+2.0*Rannyu()/m);
	return Rannyu();
}

// Generate a random integer uniformly distributed in {1,2,3,4,5,6} - Implements a fair six-faced dice
int Random::UniformDice(){
	return int(Rannyu()*6.0)+1;
}

// Generate a random polar angle theta distributed according to p(theta)=(1/2)*sin(theta) in [0,pi] - Samples the polar angle uniformly over the solid angle
double Random::Theta() {
