...
class Random {
	...
	// Random number generation
		...
		double LinearIn01(double m);					// Random number distributed according to linear pdf in [0,1) with angular coefficient m (-2<=m<=2)
		int UniformDice();								// Random number distributed according to a discrete uniform pdf with possible outcomes {1,2,3,4,5,6} (six-faced fair dice)
		double Theta();									// Random polar angle distributed according to p(theta)=(1/2)*sin(theta) in [0,pi]
};